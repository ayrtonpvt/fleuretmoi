(() => {
  ...
  function titleCore(title) {
    return normalizeTitle(title)
      .replace(/^file:/, "")
      .replace(/\b(cropped|crop|original|uncropped|retouched|edited|version|scan)\b/g, " ")
      .replace(/\([^)]*\)/g, " ")
      .replace(/\b\d{3,}\b/g, " ")
      .replace(/[^a-zà-öø-ÿ]+/g, " ")
      .replace(/\s+/g, " ")
  ...
  function areNearDuplicates(a, b) {
    const distance = hammingDistance64(a.analysis?.dhash, b.analysis?.dhash);
    if (distance <= 6) return true;
    // Cropped/original variants can move perceptual bits substantially, so allow
    // a looser visual threshold only when Commons filenames strongly agree.
    return distance <= 15 && tokenSimilarity(a.title, b.title) >= 0.72;
  }
